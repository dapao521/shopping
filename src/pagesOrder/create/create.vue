...
//获取传过来的值
const query = defineProps(['skuId', 'count'])
// console.log(query)
...
//选出默认收货地址
//如果store中有默认地址，则选中，否则选默认第一个
const addresssotore = useadressstore()
const address = computed(() => {
  return addresssotore.address || orderInfo.value?.userAddresses?.find((item) => item.isDefault)
})
...
const submitOrder = async () => {
  ...
  const { result } = await postMemberOrderAPI({
    ...
    addressId: address.value.id, //地址
    deliveryTimeType: activeDelivery.value.type, //配送时间
    buyerMessage: buyerMessage.value, //买家留言
    payType: 1, //支付方式，1为在线支付，2为货到付款
    payChannel: 2, //支付渠道：支付渠道，1支付宝、2微信--支付方式为在线支付时
  })
  // console.log(result)
  //跳转到订单页面  关闭当前页面
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <text class="small">买得起吗？</text>
    <view class="total-pay symbol">
      <text class="number">{{ orderInfo?.summary.totalPayPrice.toFixed(2) }}</text>
    </view>
    <view class="button" :class="{ disabled: !address?.id }" @tap="submitOrder"> 提交订单 </view>
  </view>
</template>
