...
import { wxPayapi, getPayMockAPI } from '@/services/pay'
...
onReady(() => {
  //页面实例中已经自带了动画函数
  // console.log(currentPage.animate)
  ...
  currentPage.animate(
    ...
    5000, // 动画持续时长
    {
      ...
      endScrollOffset: 200, // 停止滚动偏移量
      timeRange: 5000, // 时间长度
    },
  )
  // 动画效果,导航栏标题
  currentPage.animate('.navbar .title', [{ color: 'transparent' }, { color: '#000' }], 5000, {
  ...
  // 动画效果,导航栏返回按钮
  currentPage.animate('.navbar .back', [{ color: '#fff' }, { color: '#000' }], 5000, {
    scrollSource: '#scroller',
    timeRange: 5000, //时间
    startScrollOffset: 0, //开始滚动偏移量
    endScrollOffset: 200, //停止滚动偏移量
...
const getOrder = async () => {
  ...
  order.value = result
  //在获取订单的时候获取物流信息
  if ([3, 4, 5].includes(order.value?.orderState)) getLogistics()
}
...
const onPay = async () => {
  ...
  if (import.meta.env.DEV) {
    ...
    // 生产环境：获取支付参数 + 发起微信支付
    const { result } = await wxPayapi(query.id)
    // console.log(result)
...
<template>
  <!-- 自定义导航栏: 默认透明不可见, scroll-view 滚动到 200 时展示 -->
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets?.top + 20 + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button-group">
            ...
            <view v-if="isDev && order?.orderState === 2" class="button" @tap="onsend">
              模拟发货
            </view>
            <view v-if="order?.orderState === 3" class="button" @tap="onOrderConfirm">
              确认收货
            </view>
          </view>
        </template>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          <navigator
            ...
            :url="`/pagesOrder/create/create?skuId=${query.id}&count=${1}`"
            hover-class="none"
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view class="button primary" v-if="order?.orderState === 3"> 确认收货 </view>
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="order?.orderState === 4"> 去评价 </view>
          <!-- 待评价/已完成/已取消 状态: 展示删除订单 -->
          <view class="button delete" v-if="order?.orderState >= 4" @tap="onOrderDelete">
            删除订单
          </view>
        </template>
      </view>
