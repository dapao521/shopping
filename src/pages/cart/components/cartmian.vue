...
const goodslist = ref()
const getlist = async () => {
  const { result } = await getpostcartapi()
  // console.log(result)
...
//商品单个商品选中状态
const onchangeselected = async (item) => {
  // console.log(item)
...
const isSelectedAll = computed(() => {
  return goodslist.value?.length && goodslist.value.every((item) => item.selected)
})
//所有商品全选状态
const onchangeselectedAll = async () => {
  ...
  await putMemberCartSelectedAPI({ selected: _isselected })
}
...
//过滤出选中的商品
const getSelectedGoods = computed(() => {
  return goodslist.value?.filter((item) => item.selected)
})

//计算已选商品数量
const selectedGoodsCount = computed(() => {
  return getSelectedGoods.value?.reduce((total, item) => total + item.count, 0)
})

//计算总价
const totalPrice = computed(() => {
  return getSelectedGoods.value
  ...
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onscrolltolower">
    ...
    <template v-if="MemberStore.profile">
      ...
      <view class="cart-list" v-if="true">
        ...
        <uni-swipe-action>
          <!-- 滑动操作项 -->
          <uni-swipe-action-item v-for="item in goodslist" :key="item.skuId" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods">
              <!-- 选中状态 -->
              <text
            ...
            <!-- 右侧删除按钮 -->
            <template #right>
              <view class="cart-swipe-right" @tap="shanchu(item.skuId)">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" :style="{ paddingBottom: height.height + 'rpx' }">
        <text class="all" @tap="onchangeselectedAll" :class="{ checked: isSelectedAll }">全选</text>
        <text class="text">合计:</text>
        <text class="amount">{{ totalPrice }}</text>
        <view class="button-grounp" @tap="onjiesuan">
          <view class="button payment-button" :class="{ disabled: selectedGoodsCount === 0 }">
